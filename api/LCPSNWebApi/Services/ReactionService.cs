...
namespace LCPSNWebApi.Services
{
    public class ReactionService : ControllerBase, IReaction
    {
        ...
        public ReactionService(DBContext context, IConfiguration configuration)
        {
            _context = context;
        ...
        public async Task<IActionResult> ResetIdSeed(int rsid = 1)
        {
            try
            {
                ...
                return Ok(new { msg = $"Id of table Reactions has been reset to {rsid}!", qrycmd = queryString.Replace("@rsid", "" + rsid), res = result, status = 200 });
            }
            catch (Exception ex)
            {
                // Handle exceptions appropriately
